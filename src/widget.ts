...
import { branchMenuLabel } from './label';
import {
  addIcon,
  branchIcon,
  switchIcon,
...
export class KimiCodeSessionsWidget extends Widget {
  ...
  /** Menu-item variant of the above. Lumino sets no `max-width` on
   * `.lm-Menu-itemLabel`, so an uncapped auto-generated title stretches the
   * submenu across the window (DEF-18). The budget is in display columns,
   * not characters - see label.ts. */
  private _branchMenuLabel(b: IBranch): string {
    return branchMenuLabel(b.label, this._shortSessionId(b.session_id));
  }
